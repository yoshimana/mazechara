(() => {
  ...
  const text = {
    ...
    '光感・鮮やかさ（オーバーレイ）': 'Light & vibrance (Overlay)', '光源の向き': 'Light direction', '落ち影': 'Cast shadow', '影の傾き': 'Shadow tilt',
    ...
    '接地影 (足元の影)': 'Contact shadow (at feet)', '影のサイズ': 'Shadow size', 'スポットライト': 'Spotlight', 'ライト色': 'Light color', '位置を指定': 'Set position', '位置を決定': 'Confirm position',
    ...
    '2. ライトラップ (光の回り込み)': '2. Light wrap', 'ラップ強度': 'Wrap strength', '回り込み幅 (半径)': 'Wrap width (radius)', '明度閾値 (明るい背景のみ)': 'Brightness threshold (bright backgrounds only)',
    ...
    '初期化': 'Reset', '合成画像を書き出す (JPEG)': 'Export composite (JPEG)', '利用規約・注意事項': 'Terms & notices', '制作者・お問い合わせ:': 'Creator & contact:',
